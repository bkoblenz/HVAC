...
def parse_manifest(plugin):
    try:
        with open ('plugins/manifests/'+plugin+'.manifest') as mf:
            ...
            gv.logger.debug('description: ' + desc)
            f_list = [line.strip() for line in mf_list[sep+2:]]
            gv.logger.debug('file list: ', ",".join(f_list))
            return (desc, f_list)
    except IOError:
        gv.logger.error("parse_manifest IOError")
        return ('', [])


def get_readme():
    response = urllib.urlopen('https://api.github.com/repos/bkoblenz/Irricloud_plugins/readme')
    data = response.read()
    d = json.loads(data)
        ...
        if qdict['btnId'] =="del":
            ...
            for p in del_list:  # get files to delete for each plugin in list
                ...
                for f in files:
                    ...
                    gv.logger.debug('command: ' + command)
                    subprocess.call(command.split())
#            restart(1, True)
...
#           https://raw.github.com/<username>/<repo>/<branch>/some_directory/file.r #### Example
            response = urllib.urlopen('https://raw.github.com/bkoblenz/Irricloud_plugins/master/'+p+'/'+p+'.manifest')
            ...
            for f in short_list:
                pf = f.split()
                response = urllib.urlopen('https://raw.github.com/bkoblenz/Irricloud_plugins/master/'+p+'/'+pf[0])
