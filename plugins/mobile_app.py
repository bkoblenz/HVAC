...
if 'plugins.mobile_app' not in urls:
    urls.extend([
...
class options(WebPage):  # /jo
    ...
    def GET(self):
        ...
        if check_login():
            jopts = {
                "fwv": gv.ver_str+'-OSPi',
#                "tz": gv.sd['tz'], # field no longer maintained
                "ext": (gv.sd['nst']-gv.sd['radiost'])//8 - 1,
                "seq": gv.sd['seq'],
                "sdt": gv.sd['sdt'],
                "mas": gv.sd['mas'],
                "mton": gv.sd['mton'],
...
class cur_settings(ProtectedPage):  # /jc
    ...
    def GET(self):
        ...
        jsettings = {
            "devt": gv.now,
            "nbrd": (gv.sd['nst']-gv.sd['radiost'])//8,
            "en": gv.sd['en'],
            "rd": gv.sd['rd'],
            "rs": gv.sd['rs'],
            "mm": gv.sd['mm'],
...
#        dse = int((time.time()+((gv.sd['tz']/4)-12)*3600)/86400)  # days since epoch
        dse = gv.now/86400  # days since epoch
        for p in gv.pd:
            op = p[:]  # Make local copy of each program
            if op[gv.p_day_mask] >= 128 and op[gv.p_interval_day] > 1:
                rel_rem = (((op[gv.p_day_mask]-128) + op[gv.p_interval_day])-(dse % op[gv.p_interval_day])) % op[gv.p_interval_day]
                op[gv.p_day_mask] = rel_rem + 128
            lpd.append(op)
        web.header('Access-Control-Allow-Origin', '*')
        web.header('Content-Type', 'application/json')
        web.header('Cache-Control', 'no-cache')
        jpinfo = {
            "nprogs": len(gv.pd)-1,
            "nboards": (gv.sd['nst']-gv.sd['radiost'])//8,
            "mnp": 9999,
            'pd': lpd
        }
...
class get_logs(ProtectedPage):  # /jl
    ...
    def read_log(self):
        try:
            with open('./data/wlog.json') as logf:
