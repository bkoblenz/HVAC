...
from helpers import mkdir_p, jsave
...
class ETWeatherLevelChecker(Thread):
    ...
    def run(self):
        ...
        self.start_status('et_weather plugin started')
            ...
            try:
                ...
                if gv.sd['subordinate'] and not gv.sd['main']:
                    ...
                    try:
                        ...
                        self.start_status("Weather data not available.  Using no adjustment.")
                        gv.sd['wl_et_weather'] = 100.
                        self._sleep(3600)
                        continue

                    days_past = gv.sd['ethistory']
                    days_forward = gv.sd['etforecast']
...
#                        err_string = ''.join(traceback.format_exception(exc_type, exc_value, exc_traceback))
#                        gv.logger.info(err_string)
                        self.start_status('Approximating solar radiation')
		    ...
		    ETo = [penman_monteith_ETo(Rn[i], t[i], ws[i], es[i], ea[i], delta_es[i], psy) for i in range(len(Rn))]
                    ...
                    scale = [100.*(ETo[i]-total_water[i])/(float(gv.sd['etbase'])) for i in range(len(total_water))]
                    overall_scale = min(max(sum(scale)/len(scale),float(gv.sd['etmin'])), float(gv.sd['etmax']))
                    gv.sd['wl_et_weather'] = overall_scale
                    self.start_status('Scaling watering by %.1f%%\n' % (gv.sd['wl_et_weather']))
                ...
                exc_type, exc_value, exc_traceback = sys.exc_info()
                err_string = ''.join(traceback.format_exception(exc_type, exc_value, exc_traceback))
                self.start_status('Weather-base water level encountered error:\n' + err_string)
        ...
        try:
            if not os.path.exists(path) or force:
                with open(path, 'wb') as fh:
                    req = urllib2.urlopen("http://api.wunderground.com/api/"+gv.sd['etapi']+"/" + suffix)
                    while True:
...
def today_info(obj):
    ...
    get_data(request, datestring+'/history'+lid+'.json')
