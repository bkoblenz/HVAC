...
import ast
import gv
from helpers import network_up, network_exists, get_cpu_temp, jsave, reset_networking
import sys
import logging
...
class StartConfig(object):
    ...
    def run(self, port=80, *middleware):
        ...
        shutil.copy2('/etc/network/interfaces', '/etc/network/interfaces.save')
        shutil.copy2('/etc/wpa_supplicant/wpa_supplicant.conf', '/etc/wpa_supplicant/wpa_supplicant.conf.save')
...
#        create_visible_networks()
        ...
        self.logger.info('update sip_net_finish')
#        subprocess.call(['systemctl', 'enable', 'sip_net_finish.service'])
##        subprocess.call(['update-rc.d', 'sip_net_finish', 'enable'])
##        subprocess.call(['update-rc.d', 'sip_net_finish', 'defaults'])
##        self.logger.info('update sip')
#        subprocess.call(['systemctl', 'disable', 'sip.service'])
##        subprocess.call(['update-rc.d', '-f', 'sip', 'remove'])
...
if __name__ == "__main__":
    app = StartConfig()
    logger = logging.getLogger('irricloud_net_config')
    ...
    if len(sys.argv) > 1:
        ...
        if level_name in log_levels:
            ...
            logger.critical('Bad parameter to sip_net_start: ' + level_name)
